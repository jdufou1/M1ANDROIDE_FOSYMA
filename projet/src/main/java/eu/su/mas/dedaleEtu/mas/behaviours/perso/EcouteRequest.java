...
public class EcouteRequest extends OneShotBehaviour{
	...
	public void action() {
		...
		/* ROUTINE 2 : Verification si l'agent actuel est arrive a destination  */
		checkDirection();
		
		/* ROUTINE 3 : Incrémentation de l'horloge de l'agent actuel */
		this.ag.up_clock();
		
		/* ROUTINE 4 : Reinitialisation des noeuds ouverts bloqué qui nont pas pu etre visités */
		if(this.ag.getMapManager().get_fields().size() != 0 && this.ag.get_clock() % this.ag.getMapManager().get_fields().size() == 0)
			this.ag.getAgentManager().getMe().set_open_nodes_blocked(new ArrayList<Field>());
		...
		if (msgReceived!=null) {
			...
			if(ag.getAgentManager().getMe().getLastPosition() != null  &&  ag.getAgentManager().getMe().getDirection() != null &&
				...
				/* Si un interblocage est detecte */
	...
	/*
	 * Méthode qui va vérifier si l'agent actuel est arrive a destination
	 * @param null
	 * @return null
	 * */
	private void checkDirection() {
		ag.getAgentManager().getMe().set_current_position(ag.getCurrentPosition());
		if(ag.getAgentManager().getMe().getDirection() != null && 
				ag.getAgentManager().getMe().getDirection().get_position().equals(ag.getCurrentPosition())) {
			ag.getAgentManager().getMe().setDirection(null);
		}
	}
